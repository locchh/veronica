 ...
 */
export async function listArmors(): Promise<
...
> {
  ...
  for (const folder of folders) {
    ...
    const armorEntries: GitHubEntry[] = await listFolder(
      `${ARMOR_FOLDER}/${folder.name}`,
    );
    // Validate the armor
    ...
    const description = (
      await fetchRaw(`${ARMOR_FOLDER}/${folder.name}/description.md`)
    ).trim();
