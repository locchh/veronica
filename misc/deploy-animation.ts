...
// ── Sky-only frame (used in intro + loop) ──
type SkyFrameOpts = {
  capsuleRow: number;
  trailLength: number;
  capsuleChar: string;
...
// ── Ground (only shown at the final frame) ──
...
async function playLoop(durationMs: number) {
...
async function playOutro(armorName: string) {
  ...
  // unpack sequence: capsule → cracks → opens → Claude burst revealed
  const unpackFrames: Array<{ lines: string[]; status: string; pause: number }> = [
    ...
    // small sparkle emerges
    {
      lines: ["", "", `${BOLD}${BRIGHT_YELLOW}✦${RESET}`],
      status: `${DIM}Activating...${RESET}`,
      pause: 250,
    },
    // Claude Code logo emerging
    {
      lines: [
  ...
  for (const frame of unpackFrames) {
    ...
    // center the group as a block: use widest line for shared padding
...
async function deployAnimation(armorName: string, downloadMs: number) {
  process.stdout.write(HIDE_CURSOR);
  try {
    await playIntro();
    await playLoop(downloadMs);
...
await deployAnimation("mod-01", 4000);
